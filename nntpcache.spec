...
łącze. Ten program przekazuje rządzania NNTP do podanego serwera a
następnie zwraca odpowiedzi do klienta, jednocześnie zachowując je
lokalnie. Tak więc jeśli rządanie może zostać wypełnione, całkowicie
...
%attr(640,root,news) %config %{_sysconfdir}/%{name}/innreport.conf
%attr(640,root,news) %config %{_sysconfdir}/%{name}/innreport.pl
%attr(640,root,news) %config %{_sysconfdir}/%{name}/innreport.pm
%attr(640,root,news) %config %{_sysconfdir}/%{name}/newshound.conf-dist
